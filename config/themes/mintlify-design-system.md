...
colors:
  ...
  hero-dark-to: "#2d5a4f"
  testimonial-orange: "#d83e20"
  testimonial-orange-deep: "#b32f17"
  canvas: "#ffffff"
  canvas-dark: "#0a0a0a"
  surface: "#f7f7f7"
  surface-soft: "#fafafa"
...
- **Brand Tag** ({colors.brand-tag}): Documentation tag and reference color `#255bb3` (used in `<Tabs>` JSX-style annotations and code-tag chips; adjusted for 5.28:1 contrast on 15% tag bg).
- **Brand Annotate** ({colors.brand-annotate}): Inline code annotation green `#0d7d54` (used in twoslash code annotation system; adjusted for 4.99:1 contrast on white canvas).
- **Brand Warn** ({colors.brand-warn}): Code warning highlight `#965b00` (deprecated, caution; adjusted for 5.69:1 contrast on white canvas).
- **Brand Error** ({colors.brand-error}): Red `#bc2c2c` used for required-field labels and error highlight (adjusted for 5.73:1 contrast on white canvas).
- **Testimonial Orange** ({colors.testimonial-orange}): Warm coral-orange `#d83e20` used on the "Cursor" testimonial card (adjusted for 5.31:1 white copy contrast).
...
- **Stone** ({colors.stone}): Captions, twoslash cursor color, muted labels `#6e6e73` (adjusted for 5.05:1 WCAG AA contrast on white canvas).
- **Muted** ({colors.muted}): De-emphasized labels and disabled text `#646466` (adjusted for 5.81:1 WCAG AA contrast on white canvas).
...
- Background gradient `linear-gradient(180deg, {colors.hero-sky-from} 0%, {colors.hero-sky-to} 100%)`, text `{colors.primary}` (7.91:1 contrast on sky gradient for WCAG AA compliance), padding `{spacing.hero}`.
