...
export default function Footer() {
  ...
  return (
    <footer className="bg-[var(--color-bg-base)] pt-20 pb-12 border-t border-[var(--color-border)] dark:border-white/5 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-8 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
          ...
          <div className="flex flex-col">
            <h3 className="text-3xl font-serif text-[var(--color-text-main)] font-bold mb-6">
            ...
            <a
              ...
              className="inline-flex items-center text-accent font-semibold text-lg hover:text-[var(--color-text-main)] transition-colors"
            >
              Reach Out via Email
          ...
          <div className="flex flex-col md:items-end justify-end space-y-4">
            <div className="font-serif text-xl tracking-[0.2em] text-[var(--color-text-main)] font-semibold opacity-50 mb-4">
              BENJAMIN <span className="font-sans tracking-normal mx-1">//</span> FINTECH ARCHITECT
            </div>
            {/* FOOTER GROUNDING */}
            <div className="text-[var(--color-text-muted)] text-right text-sm">
              <span className="block mb-1 text-[var(--color-text-main)] font-medium">Based in Ottawa, ON | Relocated from Mexico.</span>
              Open to local and remote opportunities in FinTech and Financial Strategy.
            </div>
          </div>
        </div>

        <div className="border-t border-[var(--color-border)] dark:border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between text-sm text-[var(--color-text-muted)]">
          <p>
