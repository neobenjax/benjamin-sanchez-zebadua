...
export default function FragmentHeader({ ast }: FragmentHeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[100] glass border-b border-[var(--border-subtle)] backdrop-blur-md">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className="p-2 rounded-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
...
function RenderBrandItem({ item }: { item: FragmentLink }) {
  if (item.imageUrl) {
    return (
      <Link href={item.href} className="inline-flex items-center group">
        ...
        />
        <span className="text-lg tracking-[0.15em] text-[var(--color-text-primary)] font-semibold group-hover:text-[var(--color-accent)] transition-colors">
          {item.text || "BENJAMIN"}
        </span>
      </Link>
    );
  ...
  return (
    <Link href={item.href} className="text-[clamp(0.85rem,2.5vw,1.25rem)] leading-tight tracking-normal md:tracking-wide lg:tracking-[0.2em] text-[var(--color-text-primary)] font-semibold flex flex-wrap items-center">
      <span>{item.text}</span>
    </Link>
  );
}
  ...
  if (isMobile) {
    return (
      <Link
        href={item.href}
        onClick={onClick}
        className="block px-3 py-2.5 rounded-sm text-base font-medium text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--border-subtle)] transition-colors"
      >
        {item.text}
      </Link>
    );
...
function RenderActionButton({
  ...
  if (isMobile) {
    return (
      <a
        ...
        onClick={onClick}
        className="mt-3 w-full inline-flex items-center justify-center px-4 py-3 rounded-sm bg-[var(--color-accent)] text-[var(--color-primary)] font-bold text-base hover:brightness-110 transition-all shadow-md"
      >
  ...
  return (
    <a
      href={item.href}
      download={item.isDownload}
      className="inline-flex items-center px-5 py-2.5 rounded-sm bg-[var(--color-accent)] text-[var(--color-primary)] font-semibold text-sm hover:brightness-110 transition-all shadow-sm whitespace-nowrap"
    >
      {item.isDownload && <Download className="w-4 h-4 mr-2" />}
      {item.text}
    </a>
